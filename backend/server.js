...
// Middleware
app.use(cors({
  origin: ['https://smart-note-collab.vercel.app', 'https://smart-note-collab.vercel.app', 'https://smart-note-collab.vercel.app','http://localhost:5173'],
  credentials: true
}));
...
io.on('connection', (socket) => {
  ...
  socket.use(([event, ...args], next) => {
    ...
    if (limit.count > MAX_EVENTS_PER_SEC) {
      console.warn(`Rate limit exceeded for user ${socket.username}: ${limit.count} events/sec. Disconnecting.`);
